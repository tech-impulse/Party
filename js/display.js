                ...
                for (var i = 0; i < data.nodes.length; i++) {
                    ...
                    if ( /*(position + 1) == parseInt(data.columns) && count < filas && */ valorSwitch == 7) { //despues de la primera fila se mostrara el elemento principal
                        ...
                        if (parseInt(originNode) == 0) {
                            ...
                            var element = block + '<div style="height:' + alturaBox + 'px;width:' + heig_block + 'px;background-image:url(\'' + data.nodes[i].linkext + '\');background-position: top center;">' +
                                '<label style="font-size:35px;color: black;">' + data.nodes[i].name + '</label></div>' +
                                '</div>';
...
function displayPantallaIntermediaAsistFiestas(data) {
    ...
    htmlContent = '<div id="page_count" style="display: block;">' +
        ...
        '<div class="ui-block-c" style="width:30%;"><a data-corners="false" id="mas_fiesta" onclick="addPeople(1);" data-role="button" data-theme="b">+</a></div>' +
        '</div>' +
